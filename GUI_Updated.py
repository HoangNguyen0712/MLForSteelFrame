...
layout = [  [sg.Text('Developed by Hoang D. Nguyen')],
            ...
            [sg.Frame(layout=[
            [sg.Text('Mamimum Top Drift',size=(22, 1)),sg.InputText(key='-O1-',size=(18, 1)),sg.Text('%')],       
            [sg.Text('Mamimum Interstory Drift',size=(22, 1)), sg.InputText(key='-O2-',size=(18, 1)),sg.Text('%')]],title='Output')],
...
filename = 'BestModel_Disp.sav'
loaded_model_disp = load(open(filename, 'rb'))
filename = 'BestModel_Drift.sav'
...
while True:
    ...
    if event == 'Predict':
        ...
        if values['-f1-'] == '' or values['-f2-'] == '' or values['-f3-'] == '' or values['-f4-'] == ''  or values['-f8-'] == '' or values['-f9-'] == '' or values['-f10-'] == ''or values['-f11-'] == ''or values['-f12-'] == '':
            window['-O1-'].update('Please fill the input parameters')
            window['-O2-'].update('Please fill the input parameters')
            #print('should check')
            #break
    ...
    #print('You entered ', values['-f1-'],values['-f2-'],values['-f3-'],values['-f4-'])
            ...
            window['-O1-'].update(y_pred_disp[0]*100)
            window['-O2-'].update(y_pred_drift[0]*100)
            ...
            window['-O1-'].update(y_pred_disp[0]*100)
            window['-O2-'].update(y_pred_drift[0]*100)
    #a = int(values[0]) + int(values[1]) + int(values[2])
    #print('You entered ',a)
    #window['-OP-'].update(a)
    #print('No value')
    #update_display(values[8])
    
window.close()
#sg.Popup(event, values, 'tttt')